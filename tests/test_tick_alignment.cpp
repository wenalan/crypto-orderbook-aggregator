...
TEST(TickAlignTest, FloorBidsAndCeilAsksWithEpsilons) {
  ...
  auto a = ob_from_pairs({{99.999999999,1.0},{100.0,1.0}}, {{100.0000000001,2.0},{100.5,1.0}});
  auto b = ob_from_pairs({{100.25,3.0}}, {{100.249999999,4.0}});
...
TEST(TickAlignTest, ExactBoundaryStability) {
  ConsolidationCfg cfg; cfg.tick = 0.5; cfg.topN = 50;
  auto a = ob_from_pairs({{101.0,1.0},{100.5,2.0}}, {{101.0,3.0},{101.5,4.0}});
  ...
  auto ita = merged.asks.begin();
  EXPECT_DOUBLE_EQ(ita->first, 101.0); EXPECT_DOUBLE_EQ(ita->second, 3.0); ++ita;
  EXPECT_DOUBLE_EQ(ita->first, 101.5); EXPECT_DOUBLE_EQ(ita->second, 4.0);
}

TEST(TickAlignTest, TopNAppliesAfterAlignment) {
  ConsolidationCfg cfg; cfg.tick = 0.5; cfg.topN = 1;
  auto a = ob_from_pairs({{100.49,1.0},{99.99,2.0}}, {{100.01,1.0},{100.51,2.0}});
  auto merged = consolidate({a}, cfg);
  ASSERT_EQ(merged.bids.size(), 1u);
  ASSERT_EQ(merged.asks.size(), 1u);
  EXPECT_DOUBLE_EQ(merged.bids.begin()->first, 100.0);
  EXPECT_DOUBLE_EQ(merged.asks.begin()->first, 100.5);
}
